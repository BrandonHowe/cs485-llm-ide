...
import { VSCloneChatHistoryRail, type IVSCloneChatHistoryRailFilterState } from '../../browser/vscloneChatHistoryRail.js';
import type { IVSCloneChatHistoryRailRow } from '../../browser/vscloneChatHistoryRailTree.js';
...
function createRow(overrides: Partial<IVSCloneChatHistoryRailRow> = {}): IVSCloneChatHistoryRailRow {
	return {
		threadId: 'thread-1',
		title: 'Thread 1',
		preview: 'Preview 1',
...
suite('VSCloneChatHistoryRail', () => {
	...
	test('render initializes the loading view, header actions, and filter snapshot', () => {
		...
		try {
			...
			const searchInput = container.querySelector('.vsclone-chat-history-search') as HTMLInputElement;
			const backButton = container.querySelector('.vsclone-chat-history-back') as HTMLButtonElement;
			const newChatButton = container.querySelector('.vsclone-chat-history-new-chat') as HTMLButtonElement;
			...
			assert.strictEqual((container.querySelector('.vsclone-chat-history-list') as HTMLElement).classList.contains('hidden'), true);
			assert.strictEqual((container.querySelector('.vsclone-chat-history-state') as HTMLElement).getAttribute('role'), 'status');
			assert.strictEqual(container.querySelectorAll('.vsclone-chat-history-skeleton-row').length, 7);
		} finally {
			container.remove();
		}
	});
	...
	test('filter state updates cancel pending searches and tab clicks emit the current query snapshot', async () => {
		const { rail, container } = createHarness(store);
		try {
			const observedStates: IVSCloneChatHistoryRailFilterState[] = [];
			...
			// to avoid thrashing the history query on every keystroke.
	...
	test('rows render with selection state, accessibility labels, and click selection updates', () => {
		...
		try {
			...
			assert.strictEqual(secondRow.querySelector('.vsclone-chat-history-row-archived')?.textContent, 'Archived');
			...
			const preview = container.querySelector('[data-thread-id="thread-1"] .vsclone-chat-history-row-preview') as HTMLElement;
	...
	test('row context menus emit action events and open the delete confirmation modal', async () => {
		...
		try {
			...
			const openAction = actions.get('vsclone.chatHistory.open');
			const copyPromptAction = actions.get('vsclone.chatHistory.copyPrompt');
			const copyResponseAction = actions.get('vsclone.chatHistory.copyResponse');
			const reusePromptAction = actions.get('vsclone.chatHistory.reusePrompt');
			const toggleArchiveAction = actions.get('vsclone.chatHistory.toggleArchive');
			const deleteAction = actions.get('vsclone.chatHistory.delete');
			...
			const description = container.querySelector('.vsclone-chat-history-delete-description') as HTMLElement;
	...
	test('loading, empty, and error states render the expected markup and retry wiring', () => {
		...
		try {
			...
			rail.setRows([]);
			const stateContainer = container.querySelector('.vsclone-chat-history-state') as HTMLElement;
			assert.strictEqual(stateContainer.getAttribute('role'), 'status');
			assert.strictEqual(stateContainer.querySelector('.vsclone-chat-history-state-icon')?.textContent, '[]');
			assert.strictEqual(stateContainer.querySelector('.vsclone-chat-history-state-title')?.textContent, 'No conversations yet');
			...
			assert.strictEqual(stateContainer.querySelector('.vsclone-chat-history-state-description')?.textContent, 'Network down');
			(stateContainer.querySelector('.vsclone-chat-history-retry') as HTMLButtonElement).click();
	...
	test('delete modal traps focus and closes through cancel, backdrop, and escape', () => {
		...
		try {
			...
			assert.strictEqual(cancel, document.activeElement);
			assert.strictEqual(container.querySelector('.vsclone-chat-history-delete-description')?.textContent?.includes('Alpha'), true);
