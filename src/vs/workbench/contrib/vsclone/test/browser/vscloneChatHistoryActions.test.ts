...
function ensureChatHistoryActionsRegistered(): void {
...
suite('VSCloneChatHistoryActions', () => {
	ensureNoDisposablesAreLeakedInTestSuite();

	test('registers the chat history commands with the expected metadata and command-palette entries', () => {
		ensureChatHistoryActionsRegistered();

		const expectedTitles = new Map<string, string>([
			[VSCloneChatHistoryCommandIds.open, 'Open VSClone Chat'],
			[VSCloneChatHistoryCommandIds.focusRail, 'Focus VSClone History Rail'],
			[VSCloneChatHistoryCommandIds.toggleRail, 'Toggle VSClone History Rail'],
			[VSCloneChatHistoryCommandIds.copyPrompt, 'Copy Prompt From VSClone Thread'],
			[VSCloneChatHistoryCommandIds.copyResponse, 'Copy Response From VSClone Thread'],
			[VSCloneChatHistoryCommandIds.reusePrompt, 'Reuse Prompt From VSClone Thread'],
			[VSCloneChatHistoryCommandIds.openSession, 'Open Selected VSClone Thread'],
			[VSCloneChatHistoryCommandIds.deleteThread, 'Delete Selected VSClone Thread'],
			[VSCloneChatHistoryCommandIds.clearAllWorkspace, 'Clear VSClone Workspace Chat History'],
		]);
	...
	test('opens the view and focuses the input when the open chat action needs to open the pane', async () => {
		...
		await runCommand(VSCloneChatHistoryCommandIds.open, accessor);

		assert.deepStrictEqual(openCalls, [{ id: VSCloneViewId, focus: true }]);
		assert.deepStrictEqual(recorder.calls, ['focusInput']);
	});

	test('focuses an existing view before dispatching the view-backed history actions', async () => {
		ensureChatHistoryActionsRegistered();

		const cases = [
			[VSCloneChatHistoryCommandIds.focusRail, ['focus', 'focusRail']],
			[VSCloneChatHistoryCommandIds.toggleRail, ['focus', 'toggleRail']],
			[VSCloneChatHistoryCommandIds.copyPrompt, ['focus', 'copyPrompt']],
			[VSCloneChatHistoryCommandIds.copyResponse, ['focus', 'copyResponse']],
			[VSCloneChatHistoryCommandIds.reusePrompt, ['focus', 'reusePrompt']],
			[VSCloneChatHistoryCommandIds.openSession, ['focus', 'openSession']],
			[VSCloneChatHistoryCommandIds.deleteThread, ['focus', 'deleteActiveThread']],
		] as const;
	...
	test('delegates clearing workspace history to the history service', async () => {
		ensureChatHistoryActionsRegistered();

		const clearCalls: string[] = [];
		const historyService = {
			_serviceBrand: undefined,
			clearAll: async (scope: string) => {
				clearCalls.push(scope);
			},
		};
		const accessor = createAccessor(new Map([[IVSCloneChatHistoryService, historyService]]));

		await runCommand(VSCloneChatHistoryCommandIds.clearAllWorkspace, accessor);

		assert.deepStrictEqual(clearCalls, ['workspace']);
	});
});
